...
from authent.views import login_user, logout_user
...
from borrowing.views import (borrowing_media, borrowing_impossible, borrowing_late)
...
class TestUrlsBorrowings:
    ...
    def test_media_borrowing_url(self):
        path = reverse("borrowing_media", args=[1])
        assert resolve(path).func == borrowing_media

    def test_borrowing_impossible_url(self):
        path = reverse("borrowing_impossible")
        assert resolve(path).func == borrowing_impossible

    def test_borrowing_late_url(self):
        path = reverse("borrowing_late")
        assert resolve(path).func == borrowing_late


class TestUrlsAuthent:
    ...
    @pytest.mark.django_db
    def test_login_user_authent(self, client):
